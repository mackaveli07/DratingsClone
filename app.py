...
# Map abbreviations to TheOddsAPI full names
TEAM_NAME_MAP = {
...
def pick_bookmaker(bookmakers):
    ...
    if BOOKMAKER_PREFERENCE:
        for b in bookmakers:
            if b.get("key") == BOOKMAKER_PREFERENCE: return b
    return bookmakers[0]
...
def probability_to_moneyline(prob):
    ...
    return f"-{round(100*prob/(1-prob))}" if prob>=0.5 else f"+{round(100*(1-prob)/prob)}"
...
def fuzzy_find_team_in_odds(team_name, odds_index_keys):
    ...
    candidates=[]
    for key in odds_index_keys:
        for tk in key: candidates.append(tk)
...
def render_matchup_card(team_home, team_away, logos, odds_book,
    ...
    value_icon_svg = """<svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-star" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>"""
    ...
    st.markdown(f"<div style='text-align:center;margin-top:6px'>{edge_home_html} {edge_away_html}</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

### ---------- STREAMLIT APP ----------
st.markdown(APP_CSS, unsafe_allow_html=True)
st.title("NFL Matchup Cards")

# Load schedule and results
df_schedule = pd.read_excel(EXCEL_FILE, sheet_name=SCHEDULE_SHEET)
df_results = pd.read_excel(EXCEL_FILE, sheet_name=HIST_SHEET)

elo_ratings = run_elo_pipeline(df_results)
...
for _, row in df_schedule.iterrows():
    ...
    r_home = elo_ratings.get(team_home, BASE_ELO)
    r_away = elo_ratings.get(team_away, BASE_ELO)
    ...
    # Map abbreviations to API team names
    team_home_api = TEAM_NAME_MAP.get(team_home, team_home)
    team_away_api = TEAM_NAME_MAP.get(team_away, team_away)
    odds_key = fuzzy_find_team_in_odds(team_home_api, odds_index.keys())

    live_ml_home = live_ml_away = live_spread_home = live_spread_away = "N/A"
    odds_book = "N/A"

    if odds_key and odds_key in odds_index:
        od = odds_index[odds_key]
        live_ml_home = od["moneyline"].get(team_home_api.lower(), "N/A")
        live_ml_away = od["moneyline"].get(team_away_api.lower(), "N/A")
        live_spread_home = od["spread"].get(team_home_api.lower(), "N/A")
        live_spread_away = od["spread"].get(team_away_api.lower(), "N/A")
        odds_book = od.get("bookmaker","N/A")
    ...
    render_matchup_card(team_home, team_away, TEAM_LOGOS, odds_book,
                        ...
                        live_ml_home, live_ml_away,
                        live_spread_home, live_spread_away)
st.markdown('<div class="footer">Powered by TheOddsAPI & Elo Model</div>', unsafe_allow_html=True)
