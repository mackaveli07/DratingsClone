...
def render_matchup_card(team_home, team_away, logos, odds_book,
    ...
    st.markdown(f"<div style='text-align:center; margin-top:8px; font-weight:700'>Predicted Spread: {predicted_spread:+.1f} | Bookmaker: {odds_book}</div>", unsafe_allow_html=True)
...
for _, row in df_schedule.iterrows():
    ...
    # Odds lookup
    team_home_api = team_home
    team_away_api = team_away
    ...
    if odds_key:
        ...
        odds_book = data.get("bookmaker", "N/A")
        live_ml_home = data.get("moneyline", {}).get(team_home_api.lower(), "N/A")
        live_ml_away = data.get("moneyline", {}).get(team_away_api.lower(), "N/A")
        live_spread_home = data.get("spread", {}).get(team_home_api.lower(), "N/A")
        live_spread_away = data.get("spread", {}).get(team_away_api.lower(), "N/A")
