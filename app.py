...
def run_elo_pipeline(df):
    ...
    grouped = df.groupby(["season", "week"]) if "season" in df.columns and "week" in df.columns else [("", df)]
    for _, games in grouped:
        for _, row in games.iterrows():
            score1 = row.get("score_away", 0)
            score2 = row.get("score_home", 0)
            home_team = row.get("team_home", "")
            update_ratings(elo_ratings, row.get("team_away",""), row.get("team_home",""), score1, score2, home_team)
...
def fuzzy_find_team_in_odds(team_name, odds_index_keys):
    ...
    candidates=set([tk for k in odds_index_keys for tk in k])
...
def render_matchup_card(team_home, team_away, logos, odds_book,
    ...
    with cols[0]:
        ...
        <div class="team-block">
            <img src="{logos.get(team_away,'')}" class="team-logo"/>
            <div>
    ...
    with cols[1]:
        ...
        <div class="team-block" style="justify-content:flex-end;">
            ...
            <img src="{logos.get(team_home,'')}" class="team-logo"/>
        </div>""",unsafe_allow_html=True)
...
for _, row in sched_df.iterrows():
    team_home = row.get("team2") or row.get("team_home")
    team_away = row.get("team1") or row.get("team_away")
    r_home = ratings.get(team_home, BASE_ELO)
    r_away = ratings.get(team_away, BASE_ELO)
    prob_home = expected_score(r_home + HOME_ADVANTAGE, r_away)
    ...
    if odds_key and odds_key in odds_index:
        ...
        live_ml_home = od["moneyline"].get(team_home.lower(), "N/A")
        live_ml_away = od["moneyline"].get(team_away.lower(), "N/A")
        live_spread_home = od["spread"].get(team_home.lower(), "N/A")
        live_spread_away = od["spread"].get(team_away.lower(), "N/A")
        odds_book = od.get("bookmaker","N/A")
    render_matchup_card(
        team_home, team_away, TEAM_LOGOS, odds_book,
        prob_home, prob_away, predicted_spread,
        predicted_ml_home, predicted_ml_away,
        live_ml_home, live_ml_away,
        live_spread_home, live_spread_away
